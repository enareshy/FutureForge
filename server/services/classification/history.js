// Classification change history and centralized audit integration.
//
// Definition and assignment changes are recorded in the classification change
// history (the version-aware, queryable lineage the framework owns) AND written
// to the centralized Audit & History Framework. There is exactly one audit
// engine on the platform; this module never duplicates it.
...
export function recordChange(db, input = {}) {
  ...
  const result = run(
    db,
    `INSERT INTO cla_change_history
    ...
    [
      ...
      normalizeUpper(input.entityType || "CLASSIFICATION"),
  ...
  if (input.audit !== false) {
    try {
      writeAudit(db, {
        ...
        action: `classification.${String(input.action || "updated").toLowerCase()}`,
        resourceType: `cla_${String(input.entityType || "classification").toLowerCase()}`,
...
export function listHistory(db, { tenantId, entityType, entityId, entityRef, action, from, to, page, pageSize } = {}) {
  ...
  const total = Number(queryOne(db, `SELECT COUNT(*) AS c FROM cla_change_history ${where}`, params)?.c || 0);
  const rows = queryAll(db, `SELECT * FROM cla_change_history ${where} ORDER BY id DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
  return { items: rows.map(publicHistory), total, page: currentPage, page_size: limit, source_module: SOURCE_MODULE };
}

// Full classification lineage for one object (all assignment/values history).
export function objectLineage(db, tenantId, objectType, objectId) {
  ...
  const rows = queryAll(
    db,
    "SELECT * FROM cla_change_history WHERE tenant_id = ? AND entity_ref = ? ORDER BY id ASC",
    [Number(tenantId), ref]
  );
