// Data Security & Entitlement integration for the PDM domain.
//
// The PDM domain never trusts the client to declare authorization. Item,
// revision, dataset, CAD, rule and baseline operations are authorized against the
// centralized Data Security model and platform IAM, and object writes are
// additionally authorized against the owning/child business object type. Tenant
// and organization scope are always resolved server-side. Deny by default.
...
export function buildPdmContext(db, actor, { tenantId, organizationId = null, ip = null, correlationId = null } = {}) {
...
export function fieldDecisionsFor(db, actor, objectType, action, options = {}) {
  const context = options.context || buildPdmContext(db, actor, options);
...
// Row-level authorization of a PDM child/owning business object against the
...
// Authorizes a PDM-level operation against an IAM resource code.
export function authorizePdmAction(db, actor, { resource, action, tenantId, organizationId = null }) {
