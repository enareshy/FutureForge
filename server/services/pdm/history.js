// PDM change history and centralized audit integration.
//
// PDM item, revision, dataset, CAD, rule and baseline changes are recorded in the
// PDM change history (the queryable lineage the domain owns) AND written to the
// centralized Audit & History Framework. There is exactly one audit engine on
// the platform; this module never duplicates it.
...
export function recordChange(db, input = {}) {
  ...
  const result = run(
    db,
    `INSERT INTO pdm_change_history
    ...
    [
      ...
      normalizeUpper(input.entityType || "ITEM"),
  ...
  if (input.audit !== false) {
    try {
      writeAudit(db, {
        ...
        action: `pdm.${String(input.action || "updated").toLowerCase()}`,
        resourceType: `pdm_${String(input.entityType || "item").toLowerCase()}`,
...
export function listHistory(db, { tenantId, entityType, entityId, entityRef, action, from, to, page, pageSize } = {}) {
  ...
  const total = Number(queryOne(db, `SELECT COUNT(*) AS c FROM pdm_change_history ${where}`, params)?.c || 0);
  const rows = queryAll(db, `SELECT * FROM pdm_change_history ${where} ORDER BY id DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
  return { items: rows.map(publicHistory), total, page: currentPage, page_size: limit, source_module: SOURCE_MODULE };
}

// Full PDM lineage for one object (all item/revision/dataset history).
export function objectLineage(db, tenantId, objectType, objectId) {
  ...
  const rows = queryAll(
    db,
    "SELECT * FROM pdm_change_history WHERE tenant_id = ? AND entity_ref = ? ORDER BY id ASC",
    [Number(tenantId), ref]
  );
