// Tenant-scoped BOM configuration. Every bound (structure depth, line limits,
...
export function getConfigRow(db, tenantId, key) {
  return queryOne(db, "SELECT * FROM bom_configuration WHERE tenant_id = ? AND key = ?", [Number(tenantId), String(key)]);
}
...
export function listConfig(db, tenantId) {
  const rows = queryAll(db, "SELECT * FROM bom_configuration WHERE tenant_id = ? ORDER BY key", [Number(tenantId)]);
...
export function setConfig(db, tenantId, key, value, actor = null, ip = null) {
  ...
  if (existing) {
    run(db, "UPDATE bom_configuration SET value_json = ?, updated_by = ?, updated_at = ? WHERE id = ?", [
    ...
    run(db, "INSERT INTO bom_configuration (tenant_id, key, value_json, updated_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)", [
      Number(tenantId),
      key,
      JSON.stringify(normalized ?? null),
      actor?.id ?? null,
...
export function ensureBomConfig(db, tenantId) {
  let created = 0;
