...
- Search section, **Search & Discovery** (`/search`): centralized global/advanced search across registered object types and documents with suggestions, type/tag filters, facets, highlights, saved searches, recent searches and exports; **Enterprise search** (`/search/foundation`): the canonical provider-independent console with parsed keyword queries, structured filters and operators, AND/OR grouping, effectivity context, facets, saved/recent searches and index administration; **Search administration** (`/search/admin`): index health and queue, object-type registration, per-tenant configuration and telemetry; **Data security** (`/security`): the centralized entitlement console for object type enforcement, policies, entitlements, field security and masking, classification, organization/plant scope rules and an authorization decision inspector.
...
- Data governance section, **Governance & quality** (`/data-governance`): the centralized data governance and data quality console — data domains and catalogue, ownership and stewardship, versioned policies, quality rules and the safe rule builder, on-demand/batch evaluation, results, scores and dashboards, the exception workflow, duplicate detection and remediation.
- Data catalog section, **Data catalog** (`/data-catalog`): the centralized catalog and business glossary console — the unified metadata registry, cataloged data objects and attributes, data sources and consumers with their mappings, lineage and impact exploration, classifications, ownership and stewardship, import/export and health. **Business glossary** (`/glossary`): business terms with definitions, synonyms and relations, the review/approval lifecycle and term-to-object mappings.
- Data lifecycle section, **Data lifecycle** (`/data-lifecycle`): the centralized data lifecycle and archival console — lifecycle states and transitions, policy-scoped retention, tracked objects with state changes, the explainable eligibility engine, legal holds, archive/restore/recovery/purge, dependencies, data tiers, background jobs and configuration.
- Data exchange section, **Import & export** (`/data-exchange`): the centralized connector-based import/export framework — pluggable connectors (CSV, Excel, JSON, XML, REST, database, file, legacy PLM/ERP/CAD/MES), versioned import and export definitions with mapping/transformation/validation/filter/field-selection engines, preview and validate modes, duplicate strategies, batch jobs with checkpoints, reconciliation, per-record results and errors, idempotency, templated artifacts with expiry, unified history, metrics/health and configuration.
- Classification section, **Enterprise classification** (`/classification`): the centralized, reusable classification engine consumed across PDM/BOM/Documents/Manufacturing/Quality/Supplier/Product — classification hierarchies with materialized paths, classes with effective single-parent inheritance, typed characteristics with units, allowed values and reusable groups, class-scoped rules, multiple classification and assignment to any object, the shared validation contract, unit-aware conversion, search facets, duplicate signals, adoption metrics/coverage, background jobs, change history/lineage and per-tenant configuration (see `docs/CLASSIFICATION_DESIGN.md` and `docs/CLASSIFICATION_API.md`).
- BOM section, **BOM engine** (`/bom`): the centralized, reusable bill-of-materials capability consumed across PDM/Manufacturing/Quality/Service — BOM headers with types (EBOM/MBOM/BOP), lifecycle-aware revisions with effectivity and variant/configuration applicability, structure lines with quantity/UOM, find numbers, sequences, reference designators, usage and optional flags, substitutes, multi-level structure trees, recursive quantity rollup, where-used and uses analysis, revision/baseline comparison, frozen immutable baselines, declarative EBOM-to-MBOM transformation, configurable validation rules, search, metrics/health, background jobs, change history/lineage and per-tenant configuration (see `docs/BOM_DESIGN.md` and `docs/BOM_API.md`).
...
- `/api/v1/search` — Enterprise Search Foundation (canonical, provider-independent): `POST` search/`count`/`parse`/`bulk`, `/objects` (+ `/:objectType`), `/facets`, `/suggestions`, `/fields`, saved searches and history; `/api/v1/search/index` (+ `/rebuild` `/status` `/jobs` `/retry-failed`) — indexing administration; `/api/v1/search/content-text` — extracted text; `/api/v1/search/meta` `/health` `/metrics` — vocabulary and telemetry (see `docs/SEARCH_FOUNDATION_API.md`)
- `/api/v1/security` — Data Security & Entitlement Model: `/vocabulary` `/overview` `/cache/invalidate`; `/object-types` (+ `/:objectType`, `/status`) — enforcement registration; `/policies` (+ `/:id`, `/status`), `/entitlements` (+ `/:id`, `/status`); `/field-rules` (+ `/:id`, `/status`) and `/masking-rules` (+ `/:id`, `/status`); `/classification-rules`, `/organization-rules` and `/plant-rules` (+ `/status`); `/decisions` (+ `/context/:userId`, `/evaluate`, `/evaluate/batch`) and `/api/v1/authorization/check` `/batch-check` — authorization debugger with batch evaluation; `/api/v1/entitlements` aliases (see `docs/DATA_SECURITY_API.md`)
- `/api/v1/data-governance` — Data Governance: `/meta` `/metrics` `/health`; `/domains` (+ `/tree`, `/:ref`, `/status`), `/ownership` (+ `/resolve`), `/catalog` (+ `/:ref`, `/attributes`), `/policies` (+ `/:ref`, `/status`, `/versions`), `/configuration`, `/dimensions` (+ `/:code`) and `/jobs` (+ `/evaluate`, `/duplicates`) (see `docs/DATA_GOVERNANCE_API.md`)
- `/api/v1/data-quality` — Data Quality: `/meta`; `/rules` (+ `/validate`, `/:ref`, `/versions`, `/status`), `/evaluate` (+ `/batch`), `/results` (+ `/:objectType/:objectId`, `/history`), `/violations`, `/scores` (+ `/domains`, `/object-types`, `/trend`), `/exceptions` (+ `/summary`, `/:ref`, `/assign`, `/status`, `/comments`), `/duplicates/match-rules` `/duplicates/candidates` `/duplicates/detect` `/duplicates/summary` and `/remediations` (+ `/summary`) (see `docs/DATA_GOVERNANCE_API.md`)
- `/api/v1/data-catalog` — Data Catalog: `/meta` `/metrics` `/health`; `/entries` (+ `/:ref`, `/versions`, `/status`), `/objects` (+ `/:ref`, `/status`, `/:ref/attributes`), `/attributes` (+ `/:ref`, `/status`), `/sources` (+ `/:ref`, `/status`, `/:ref/mappings`), `/source-mappings` (+ `/:id`), `/consumers` (+ `/:ref`, `/status`, `/:ref/mappings`), `/consumer-mappings` (+ `/:id`), `/lineage` (+ `/graph`, `/impact`, `/:id`), `/relationship-types` `/relationships`, `/classifications` (+ `/status`) `/classification-assignments`, `/ownership` (+ `/gaps`, `/resolve`, `/:id`), `/configuration` (+ `/:key`), `/import-runs` (+ `/:id`) `/import` (+ `/submit`) `/export` (+ `/submit`) and `/reindex` (see `docs/CATALOG_API.md`)
- `/api/v1/glossary` — Business Glossary: `/meta` `/metrics`; `/terms` (+ `/export`, `/:ref`, `/submit`, `/approve`, `/reject`, `/status`, `/:ref/definitions`, `/:ref/synonyms`, `/:ref/relations`, `/:ref/mappings`), `/term-relations` (+ `/:id`) and `/term-mappings` (+ `/:id`, `?target_type=&target_id=`) (see `docs/CATALOG_API.md`)
- `/api/v1/lifecycle` — Data Lifecycle & Archival: `/meta` `/health` `/metrics` `/providers`; `/states` (+ `/:code`, `/:code/transitions`, `/status`), `/transitions` (+ `/:id/status`), `/tiers` (+ `/:stateCode`); `/policies` (+ `/resolve`, `/:ref`, `/versions`, `/:ref/status`); `/objects` (+ `/:objectType/:objectId`, `/snapshot`, `/history`, `/state`, `/retention`, `/tier`, `/eligibility`, `/dependencies`, `/archive`, `/cold-storage`, `/restore`); `/eligibility` (`/check`, `/batch`, `/due`); `/legal-holds` (+ `/:ref`, `/:ref/release`, `/:ref/cancel`); `/dependencies` (+ `/refresh`, `/:id/resolve`); `/archives` (+ `/:ref`, `/:ref/verify`), `/restores` (+ `/:ref`, `/:ref/execute`), `/recoveries` (+ `/:ref`, `/:ref/execute`), `/purges` (+ `/summary`, `/:ref`, `/evaluate`); `/history`, `/catalog-types`, `/snapshots`, `/configuration` (+ `/:key`) and `/jobs` (+ `/evaluate`, `/archive`, `/cold-storage`, `/restore`, `/purge`, `/recovery`, `/maintenance`) (see `docs/DATA_LIFECYCLE_API.md`)
- `/api/v1/data-exchange` — Import & Export Framework: `/meta` `/health` `/metrics` `/connectors`; `/connector-configurations` (+ `/:ref`, `/status`, `/test`, `/discover`), `/credential-references` (+ `/:ref/status`); `/import-definitions` (+ `/:ref`, `/status`, `/versions`, `/catalog`, `/validate`, `/preview`, `/run`), `/import-jobs` (+ `/:ref`, `/run`, `/reconcile`, `/retry`, `/cancel`, `/records`, `/errors`), `/import-errors`; `/export-definitions` (+ `/:ref`, `/status`, `/versions`, `/catalog`, `/validate`, `/preview`, `/run`), `/export-jobs` (+ `/:ref`, `/run`, `/cancel`, `/results`), `/export-results` (+ `/:ref/download`); `/templates` (+ `/:ref`, `/status`, `/versions`, `/validate`); `/jobs`, `/history` and `/configuration` (+ `/:key`) (see `docs/DATA_EXCHANGE_API.md`)
- `/api/v1/migration` — Migration & Onboarding Framework: `/meta` `/health` `/metrics`; `/source-adapters`, `/source-configurations` (+ `/:ref`, `/status`, `/test`, `/discover`); `/projects` (+ `/:ref`, `/status`, `/packages`, `/dependencies`, `/topology`, `/readiness`, `/plans`), `/plans` (+ `/:ref`, `/approve`); `/packages` (+ `/:ref`, `/status`, `/dependencies`, `/readiness`, `/preview`, `/validate`, `/jobs`); `/definitions` (+ `/:ref`, `/status`, `/validate`, `/versions`, `/jobs`); `/jobs` (+ `/:ref`, `/batches`, `/results`, `/errors`, `/checkpoints`, `/lineage`, `/cancel`, `/pause`, `/resume`, `/retry`, `/execute`, `/reconcile`, `/submit-reconcile`, `/files/retry`); `/identifier-mappings` (+ `/bulk`, `/resolve`), `/relationship-mappings`, `/relationships` (+ `/bulk`, `/retry`), `/file-migrations`, `/reconciliations` (+ `/:ref`, `/:ref/exceptions`), `/statistics`, `/audit`, `/object-lineage`, `/configuration` (+ `/:key`), `/maintenance` and `/seed` (see `docs/MIGRATION_API.md` and `docs/MIGRATION_DESIGN.md`)
- `/api/v1/classification` — Enterprise Classification Framework: `/meta` `/health` `/metrics` `/coverage` `/config` (+ `/:key`); `/classifications` (+ `/:ref`, `/status`, `/approve`, `/versions`, `/audit`, `/tree`, `/reorder-classes`); `/classes` (+ `/:ref`, `/status`, `/move`, `/copy`, `/children`, `/ancestors`, `/descendants`, `/versions`, `/effective`, `/characteristics`, `/validate`, `/approved-values`), `/class-characteristics/:ref`; `/characteristics` (+ `/:ref`, `/status`, `/versions`, `/allowed-values`), `/allowed-values/:ref`, `/groups` (+ `/:ref`, `/members`, `/members/:characteristicRef`), `/rules` (+ `/:ref`); `/assignments` (+ `/:ref`, `/values`, `/status`, `/reclassify`, `/validate`); `/objects/:objectType/:objectId` (+ `/values`, `/effective`, `/validate`), `/objects/:objectType/validate-batch`; `/duplicates/scan`, `/duplicates/summary`, `/units`, `/units/convert`; `/history`, `/lineage/:objectType/:objectId`; `/jobs/bulk-assign`, `/jobs/bulk-validate`, `/jobs/duplicate-scan`, `/jobs/maintenance` and `/seed` (see `docs/CLASSIFICATION_API.md` and `docs/CLASSIFICATION_DESIGN.md`)
- `/api/v1/bom` — BOM Engine: `/meta` `/health` `/metrics` `/compare-summary` `/config` (+ `/:key`) `/units` `/units/convert`; `/boms` (+ `/:ref`, `/status`, `/audit`, `/revisions`), `/revisions` (+ `/:ref`, `/status`, `/revise`, `/tree`, `/structure`, `/lines`, `/lines/reorder`, `/substitutes`, `/substitutes/summary`, `/rollup`, `/validate`, `/validation-results`); `/lines` (+ `/:ref`, `/attributes`), `/substitutes` (+ `/:ref`); `/where-used` (+ `/:objectId`, `/:objectId/multi-level`, `/:objectId/summary`), `/uses`; `/compare`, `/comparisons` (+ `/:ref`, `/:ref/results`); `/transformations` (+ `/:ref`, `/mappings`, `/mappings/:mappingId`), `/transform`, `/transformation-runs` (+ `/:ref`); `/validation-rules` (+ `/:ref`), `/validation-results` (+ `/:ref`, `/:ref/issues`); `/baselines` (+ `/:ref`, `/:ref/lines`, `/:ref/snapshot`, `/:ref/freeze`); `/history`, `/history/:objectType/:objectId`; `/jobs/rollup`, `/jobs/where-used`, `/jobs/transform`, `/jobs/validate`, `/jobs/compare`, `/jobs/maintenance`, `/foundation/ensure`, `/seed`, `/search-meta`, `/search/reindex` (see `docs/BOM_API.md` and `docs/BOM_DESIGN.md`)
- `/api/v1/pdm` — PDM domain: `/meta` `/health` `/metrics` `/rule-usage` `/config` (+ `/:key`); `/items` (+ `/:ref`, `/status`, `/audit`, `/revisions`, `/structure`, `/where-used`, `/datasets`), `/parts`, `/products`; `/revisions` (+ `/:ref`, `/status`, `/revise`, `/validate`, `/validation-results`, `/datasets`, `/representations`, `/design-data`, `/cad`); `/datasets` (+ `/:ref`, `/status`, `/content`), `/representations` (+ `/:ref`), `/design-data` (+ `/:ref`), `/cad-associations` (+ `/:ref`); `/revision-rules` (+ `/resolve`, `/:ref`, `/activate`, `/status`, `/versions`), `/configuration-rules` (+ `/evaluate`, `/:ref`, `/activate`, `/status`, `/versions`); `/baselines` (+ `/:ref`, `/release`, `/freeze`, `/retire`, `/snapshot`, `/members`, `/members/:memberId`); `/relationships` (+ `/:ref`), `/references` (+ `/summary`), `/where-used` (+ `/:ref`), `/where-referenced` (+ `/:targetType/:targetId`), `/structure/:ref` (+ `/validate`, `/structure/resolve`); `/validation-rules` (+ `/:ref`), `/validation/run`, `/validation-results` (+ `/:ref`); `/history`, `/history/:objectType/:objectId`; `/jobs/structure`, `/jobs/where-used`, `/jobs/where-referenced`, `/jobs/baseline`, `/jobs/validate`, `/jobs/reindex`, `/jobs/maintenance`, `/foundation/ensure`, `/seed`, `/search-meta`, `/search/reindex` (see `docs/PDM_API.md` and `docs/PDM_DESIGN.md`)
