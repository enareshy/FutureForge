...
The first start automatically creates `data/iam.db`, applies all migrations (`001_iam_core` through `020_integration_framework`) and seeds demo data.
...
Workers shut down gracefully on `SIGTERM`. See `docs/JOB_EXECUTION_OPERATIONS.md` for tuning, dead letters and troubleshooting. Workers also run the integration housekeeping sweep (queued messages, event fan-out and outbound webhooks); see `docs/INTEGRATION_OPERATIONS.md`.
