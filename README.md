...
Workers shut down gracefully on `SIGTERM`. See `docs/JOB_EXECUTION_OPERATIONS.md` for tuning, dead letters and troubleshooting. Workers also run the integration housekeeping sweep (queued messages, event fan-out and outbound webhooks); see `docs/INTEGRATION_OPERATIONS.md`. Workers drive the Event & Messaging Framework (outbox publish, delivery consumption, replay and retention); see `docs/EVENTS_OPERATIONS.md`.
...
- `/api/events/outbox` (+ `/stats`, `/process`, `/:id/retry`) and `/api/events/dead-letters` (+ `/stats`, `/:id`, `/resolve`) — transactional outbox and dead-letter recovery
...
- `/api/events/monitoring` (`/dashboard`, `/throughput`, `/failures`, `/latency`, `/health`, `/ordering`, `/traceability`) — operational dashboards and traceability
